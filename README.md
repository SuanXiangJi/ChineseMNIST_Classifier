...
| ----------------------------- | ---------- | ---------- | -------- | ---------- |
| MLP (基础)                    | 98.5%      | 92.3%      | 1.4M     | 6.2%       |
| MLP v1 (GELU + LN)            | 99.2%      | 94.8%      | 1.4M     | 4.4%       |
| MLP v2 (Residual + GELU + LN) | 99.5%      | 95.6%      | 2.8M     | 3.9%       |
| MLP Ablation GELU Only        | 98.8%      | 93.5%      | 1.4M     | 5.3%       |
| MLP Ablation LayerNorm Only   | 98.9%      | 93.9%      | 1.4M     | 5.0%       |
| CNN                           | 99.7%      | 96.8%      | 2.1M     | 2.9%       |

## 实验结论

1. **CNN模型表现最佳**：在所有模型中，CNN模型的测试准确率最高（96.8%），过拟合程度最低。

2. **MLP变体性能提升**：
   - MLP v1（GELU + LayerNormalization）比基础MLP提高了2.5%的测试准确率
   - MLP v2（添加残差连接）进一步提高了0.8%的测试准确率
   - 残差连接、GELU激活和LayerNormalization的组合使用效果最佳

3. **消融实验结果**：
   - 仅使用GELU激活比基础MLP提高了1.2%的测试准确率
   - 仅使用LayerNormalization比基础MLP提高了1.6%的测试准确率
   - 两者结合使用效果更好，说明它们有互补作用

4. **模型复杂度与性能权衡**：
   - MLP v2虽然参数数量最多，但准确率提升明显
   - CNN模型参数数量适中，但性能最优
